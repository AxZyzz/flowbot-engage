...
const Bullet: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <li className="flex items-start gap-2 text-sm text-muted-foreground"><span className="mt-0.5 inline-flex size-5 items-center justify-center rounded-md bg-primary/10 text-primary"><Check size={14} /></span><span>{children}</span></li>
);
...
const ServiceShowcase: React.FC = () => {
  return (
    <section id="solutions" aria-label="Solutions" className="mx-auto max-w-6xl space-y-24">
      ...
      <Reveal>
        <article className="grid items-center gap-10 md:grid-cols-2">
          <div>
            ...
            <p className="mt-3 text-muted-foreground">Broadcast campaigns, trigger smart replies, and handle support automatically while your team focuses on growth.</p>
      ...
      <Reveal>
        <article className="grid items-center gap-10 md:grid-cols-2">
          <figure className="order-last glass rounded-2xl p-3 md:order-first">
            <img
              src={flowDiagram}
